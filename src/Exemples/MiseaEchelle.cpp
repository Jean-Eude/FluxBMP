...
int main(int argc, char **argv) {
    ...
    // Transformation géométrique (échelle)
    for (int y = 0; y < bmp.getHeight(); y++) {
        for (int x = 0; x < bmp.getWidth(); x++) {
            ...
            int x_geom = static_cast<int>(lambda * (x - bmp.getWidth() / 2.) + 0.0 * (y - bmp.getHeight() / 2.) + bmp.getWidth() / 2.);
            int y_geom = static_cast<int>(0. * (x - bmp.getWidth() / 2.) + lambda * (y - bmp.getHeight() / 2.) + bmp.getHeight() / 2.);
